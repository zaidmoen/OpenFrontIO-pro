...
export interface AttackLogicInput {
  ...
  /** Attacking snipers are positioned within range to support the advance. */
  attackerHasSniperSupport?: boolean;
  /** Every passable land exit around the tile is controlled by the attacker. */
  defenderEncircled?: boolean;
...
export class Config {
  ...
  attackLogic(input: AttackLogicInput): AttackLogicResult {
    ...
    if (defender !== null && input.attackerHasSniperSupport) {
      // Snipers soften exposed positions so infantry can advance with fewer
      // losses. Their support is deliberately modest and does not apply to
      // attacks on unclaimed land.
      mag *= 0.9;
      tileCost *= 0.9;
    }
    if (defender !== null && input.defenderEncircled) {
      // Cut-off defenders have no safe route for reinforcements or retreat.
      mag *= 0.85;
      tileCost *= 0.8;
    }
    ...
    const defenderTroopLoss =
      ...
      (input.defenderEncircled ? 1.35 : 1);
